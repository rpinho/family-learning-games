...
if(command==='setup'){
 ...
 for(const [i,[game,key]]of games.entries()){
  ...
  const child=spawn(process.execPath,['server.mjs'],{cwd:resolve(root,'games',game),stdio:'inherit',env:{...process.env,[key]:data,HOST:process.env.HOST||'127.0.0.1',PORT:String(port)}});children.push(child);
