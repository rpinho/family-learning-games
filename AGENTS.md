...
3. In the repository root, run **`npm run play`**. This installs locked dependencies, builds the two React apps, and starts all six servers. Keep the process running. For a split workflow use `npm run setup`, then `npm start`.
4. Verify HTTP success for each root page at ports **4811–4816** and the links/assets referenced by each page. Check `/api/beginner/state` for Letter Quest, `/api/beginner` for Word Arcade and Number Park, and `/api/state?player=beginner` for the other three. These reads should produce fresh generic profiles on a new installation. Do not create test scores in a child's preset; use Admin if writes are necessary.
5. Return the six localhost links and explain that the server terminal must remain open. A different device needs trusted-LAN access; only enable `HOST=0.0.0.0` if the user wants it. Never create a public tunnel, public host, firewall exception or router port forward as a default installation step. Do not promise a publicly playable URL or guaranteed offline/PWA behavior.
...
- If ports are occupied, do not kill unrelated processes. Use a free six-port block via `BASE_PORT` and provide the adjusted URLs.
...
- `npm test` runs the six suites sequentially; some tests use fixed isolated ports. Build before testing. For code changes, run the relevant tests and `npm run check:privacy` before any public commit.
