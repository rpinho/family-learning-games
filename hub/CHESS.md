...
Rook gives the short task instruction on every new board, even when the preceding board had the same goal. Task instructions bypass chatter suppression; other automatic speech still suppresses repeats for 90 seconds and is spaced at least 15 seconds apart. First moves also names a successfully made fork aloud. Routine correct moves, mistakes, restarts and assisted finishes use visual feedback without automatic commentary. Requested hints still speak; the same hint line is suppressed for 20 seconds. Tapping Rook always replays the guidance, including when coaching is muted. Lesson stars distinguish independent solves (filled) from assisted practice (outlined); hints never cost lives or block play.
