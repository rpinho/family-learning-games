...
const files=['index.html','hub.mjs','style.css','bridge.mjs','dribble-ui.mjs','pitch.mjs','icon.svg','icon-192.png','icon-512.png','manifest.webmanifest'];
const server=http.createServer(async(req,res)=>{
 ...
 try{
  ...
  if(u.pathname==='/health')return send(res,200,{ok:true,version:VERSION,diagnostics:{ok:!logError,error:logError}});
  if(/^\/voice\/(manifest\.json|[a-f0-9]{16}\.wav)$/.test(u.pathname)&&req.method==='GET'){try{const bytes=await readFile(join(data,u.pathname.slice(1)));res.writeHead(200,{'Content-Type':u.pathname.endsWith('.wav')?'audio/wav':'application/json'});res.end(bytes);}catch(e){if(e.code==='ENOENT')send(res,404,{error:'Use device narration.'});else throw e;}return;}
  if(u.pathname==='/api/config'&&req.method==='GET')return send(res,200,{players:config.players,version:VERSION});
  ...
  if(icon&&icons[icon[1]])file=join(root,icons[icon[1]]);else if(u.pathname==='/dribble.mjs')file=join(here,'dribble.mjs');else{const name=u.pathname==='/'?'index.html':u.pathname.slice(1);if(!files.includes(name))return send(res,404,{error:'Not found.'});file=join(here,'public',name);}
