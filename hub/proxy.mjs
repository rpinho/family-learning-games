...
export function proxy(req,res,{port,prefix,path,player,players,game},log){
 ...
 upstream.on('error',e=>{void log({type:'proxy_error',game,player,detail:e.message});if(!res.headersSent)res.writeHead(503,{'Content-Type':'text/html'});res.end('<p>This game is starting or unavailable. Use Games above, then try again.</p>');});
