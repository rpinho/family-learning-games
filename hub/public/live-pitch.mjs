...
import {GOAL} from './dribble-live.mjs';
export function createLivePitch(canvas){
 ...
 return {draw(s,{paused=true,target=null,ready=false}={}){
  ...
  // These posts and net match the scoring region in the physics engine.
  c.fillStyle=s.outcome==='goal'?'#ffd75a55':'#071e3590';c.fillRect(GOAL.left,GOAL.back,GOAL.right-GOAL.left,GOAL.line-GOAL.back);
  for(let x=GOAL.left+16;x<GOAL.right;x+=16)line(x,GOAL.back,x,GOAL.line,'#c8e8f577',1);
  for(let y=GOAL.back+12;y<GOAL.line;y+=12)line(GOAL.left,y,GOAL.right,y,'#c8e8f577',1);
  line(GOAL.left,GOAL.line,GOAL.left,GOAL.back,'#fff',7);line(GOAL.left,GOAL.back,GOAL.right,GOAL.back,'#fff',7);line(GOAL.right,GOAL.back,GOAL.right,GOAL.line,'#fff',7);
  line(GOAL.left,GOAL.line,GOAL.right,GOAL.line,'#fff',3);disc(GOAL.left,GOAL.line,6,'white');disc(GOAL.right,GOAL.line,6,'white');
  c.strokeStyle='#c6ffe699';c.lineWidth=2;c.strokeRect(240,90,320,110);label(title,right-60,592,14,'#c5eddb');
  ...
  const bx=shown.bx,by=s.outcome==='goal'?Math.max(GOAL.back+16,shown.by-22):shown.by;
  disc(bx,by+3,13,'#082f4244');disc(bx,by,12,'white');disc(bx,by,4,'#173245');for(let i=0;i<5;i++)disc(bx+Math.cos(i*1.256)*8,by+Math.sin(i*1.256)*8,2.3,'#173245');
  if(s.outcome||paused){const text=s.outcome==='goal'?'GOAL!':s.outcome==='escaped'?'PAST THE DEFENDER!':s.outcome==='tackled'?'Ball taken. Try a new move.':s.outcome==='practice'?'Take a breather.':ready?'Drag anywhere to dribble':'Paused';c.fillStyle='#102b3bd9';c.beginPath();c.roundRect(120,420,560,70,22);c.fill();label(text,400,464,s.outcome==='goal'?42:s.outcome==='tackled'?25:28,['goal','escaped'].includes(s.outcome)?'#ffe388':'#fff');}
 },reset(){shown=null;}};
}
