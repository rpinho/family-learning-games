...
test('The bridge doubles Small steps with 54 full-board lessons and separate examples',()=>{
 assert.equal(BRIDGE_UNITS.length,18);
 assert.equal(STEP_UNITS.length,36);
 assert.equal(STEP_LESSONS.length,108);
 const practice=BRIDGE_UNITS.flatMap(u=>STEPS[u.theme]);
 const bridgeLessons=STEP_LESSONS.filter(l=>l.unit>=18);
...
test('Existing completions unlock the bridge without changing an active final old lesson',()=>{
 ...
 assert.equal(path.find(x=>x.current).id,'steps-real-fork-1');
