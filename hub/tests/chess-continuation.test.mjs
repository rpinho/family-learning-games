...
test('Eight appended units provide 120 new positions and 24 separate demonstrations',()=>{
 assert.equal(STEP_UNITS.length,12);assert.equal(STEP_LESSONS.length,36);assert.equal(extra.length,120);
 const examples=Object.values(STEP_EXAMPLES).filter(p=>p.id.startsWith('continuation-'));
 assert.equal(examples.length,24);const all=[...extra,...examples];assert.equal(new Set(all.map(p=>p.fen)).size,144);
 for(const p of all){const b=new Chess(p.fen);assert.equal(b.isCheck(),false);const pieces=b.board().flat().filter(Boolean);assert(pieces.length<=7);
  ...
  if(p.theme==='stepsMateTwo')assert(b.isCheckmate());
 }
});
test('A learner who finished the original course gets unit 5 immediately, with old work untouched',()=>{
 ...
 assert.equal(path.find(x=>x.current).id,'steps-value-1');assert.equal(path.filter(x=>x.complete).length,12);assert.equal(path.filter(x=>x.locked).length,23);assert.deepEqual(p,before);
});
 ...
 for(const p of [...extra,...Object.values(STEP_EXAMPLES)].filter(p=>p.nextLines)){
  ...
  for(const [first,branch]of Object.entries(p.nextLines)){
   ...
   for(const reply of replies){b.move(reply);const finishes=b.moves({verbose:true}).filter(end=>{
    if(p.theme==='stepsSkewer'&&end.captured!=='r')return false;
    b.move(end);const goal=p.theme==='stepsMateTwo'?b.isCheckmate():!b.moves({verbose:true}).some(r=>r.to===end.to&&r.captured);b.undo();return goal;
...
test('Alternative first checks and finishes survive reload, hints, rollback and solved-board replay',async()=>{
 for(const puzzle of extra.filter(p=>p.nextLines))for(const [first,branch]of Object.entries(puzzle.nextLines))for(const finish of branch.finishes){
  let p=freshChess();p.settings.band='steps';p.session={id:'continuation-test',lesson:puzzle.theme==='stepsMateTwo'?'steps-mate-two-1':'steps-skewer-1',band:'steps',ids:[puzzle.id],index:0,ply:0,hints:0,errors:0,results:[],phase:'puzzle'};
  ...
  await act(p,'hint');assert.match(publicChess(p).session.feedback.voice,puzzle.theme==='stepsMateTwo'?/checkmate/:/rook safely/);
