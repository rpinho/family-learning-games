...
test('Fourteen appended units provide 210 new positions and 42 separate demonstrations',()=>{
 assert.equal(STEP_UNITS.length,18);assert.equal(STEP_LESSONS.length,54);assert.equal(extra.length,210);
...
test('A learner who finished the original course gets unit 5 immediately, with old work untouched',()=>{
 ...
 assert.equal(path.find(x=>x.current).id,'steps-value-1');assert.equal(path.filter(x=>x.complete).length,12);assert.equal(path.filter(x=>x.locked).length,41);assert.deepEqual(p,before);
});
