...
import {isDragCookie} from '@/lib/cookie-division.mjs';

type Props={q:any;draft:(number|null)[];disabled:boolean;result:any;message?:string;helped:boolean;place:(draft:(number|null)[])=>void;check:(guess?:{perPlate:number;leftover:number})=>void;report:(name:string,detail:string)=>void;ask?:{options:number[];tries:number}|null;answer?:(value:number)=>void};
...
export function CookieDivision({q,draft,disabled,result,message,helped,place,check,report,ask,answer}:Props){
 ...
 const put=(id:number,plate:number|null)=>{if(disabled||result||ask)return;if(bags&&plate!==null&&spots.filter(v=>v===plate).length>=q.bagSize)return;const next=[...spots];next[id]=plate;setSelected(null);place(next);report('place',`${plate===null?'back':plate}: ${next.filter(v=>v!==null).length}/${q.total}`);};
 ...
 const bags=q.mode==='bags';
 const fixing=q.mode==='fix'||q.mode==='mixed',hide=(!!q.hideCounts||!!ask)&&!result;
 ...
 const tip=selected!==null?`Now tap the ${bags?'bag':'plate'} it should go to.`:bags?(helped?'Fill one bag until it is full. Then start the next bag.':'Drag cookies into the bag. When it is full, a new bag appears.'):helped?(q.mode==='fix'?'Find the fullest plate. Move one cookie to the plate with the fewest.':q.mode==='mixed'?'Tray cookies go to the smallest plates first.':'One on each plate. Go around again.'):fixing?(q.mode==='fix'?'Drag a cookie from one plate to another, or tap a cookie then a plate.':'Drag cookies from the tray or between plates. The plates hide their numbers.'):'Tap a plate to deal the next cookie, or drag one from the tray.';
 ...
 return <section className={"cookie-game"+(fixing?" cookie-game-fix":"")} aria-label="Share the cookies equally">
  <div className={'cookie-scene'+(fixing?' cookie-scene-fix':'')}><CookieBuddy shared={q.total-remaining} happy={!!result}/>{fixing&&!result&&<p className="cookie-bubble">{q.mode==='fix'?'Oops! I piled them up wrong. Can you make it fair?':'I dropped some! Count carefully and make it fair.'}</p>}{bags?<div className="cookie-count"><span className="cookie-mark" aria-hidden="true">🍪</span><strong className="eq-total">{q.total}</strong><span>cookies</span><span className="cookie-divider">·</span><strong className="eq-size">{q.bagSize}</strong><span>in each bag</span></div>:<div className="cookie-count"><span className="cookie-mark" aria-hidden="true">🍪</span><strong className="eq-total">{q.total}</strong><span>cookies</span><span className="cookie-divider">÷</span><strong className="eq-size">{q.plates}</strong><span>{fixing||q.mode==='share'?'friends':'plates'}</span></div>}</div>
  <div className="cookie-plates" aria-label="Plates">
   {visible.map(plate=>{const count=counts[plate];return <button type="button" key={plate} data-cookie-plate={plate} className={(bags?'cookie-bag ':'')+(bags&&count===q.bagSize?'cookie-bag-full ':'')+'cookie-plate '+(!bags&&full&&!equal&&(!fixing||helped)&&count!==Math.min(...counts)?'cookie-plate-more ':'')+(result?'cookie-plate-done':'')+(selected!==null&&spots[selected]!==plate?' cookie-plate-target':'')} disabled={disabled||!!result} aria-label={bags?`Bag ${plate+1}, ${count} of ${q.bagSize} cookies`:hide?`Friend ${plate+1}'s plate. Count the cookies.`:`Friend ${plate+1}'s plate, ${count} ${count===1?'cookie':'cookies'}. Tap to place a cookie.`} onClick={()=>{const id=selected!==null&&spots[selected]!==plate?selected:spots.indexOf(null);if(id>=0)put(id,plate)}}>
    <span className="cookie-plate-label">{bags?<span className="cookie-bag-tag" aria-hidden="true">🛍️</span>:<span className={'cookie-friend cookie-friend-'+plate%5} aria-hidden="true"><i/><i/><b/></span>}{bags?`BAG ${plate+1}`:`FRIEND ${plate+1}`}</span><span className="cookie-plate-bowl">{spots.map((at,id)=>at===plate?<span key={id} role="button" aria-label={`Move a cookie from friend ${plate+1}`} className={'cookie-on-plate'+(selected===id?' cookie-selected':'')+(drag?.id===id&&moved.current?' cookie-lifted':'')} {...grab(id)}><span className="cookie-shape" style={fixing?tilt(id):undefined}/></span>:null)}</span>{bags?<span className="cookie-bag-slots" aria-hidden="true">{Array.from({length:q.bagSize},(_,i)=><i key={i} className={i<count?'on':''}/>)}</span>:<span className={'cookie-plate-count eq-each'+(hide?' cookie-plate-count-hidden':'')}>{hide?'?':count}</span>}
   </button>})}
  </div>
  {((q.mode!=='fix'&&!bags)||remaining>0)&&<div className="cookie-tray"><div className="cookie-tray-top"><strong>Cookie tray</strong><span>{remaining} left · tap or drag to a {bags?'bag':'plate'}</span></div><div className="cookie-tray-pieces">{spots.map((at,id)=>at===null?<button type="button" key={id} className={'cookie-piece '+(selected===id?'cookie-selected':'')} disabled={disabled||!!result} aria-label={`Cookie ${id+1}, tap then choose a plate or drag it`} {...grab(id)}><span className="cookie-shape" aria-hidden="true"/></button>:null)}</div></div>}
  <p className="cookie-tip">{tip}</p>
  {!fixing&&!ask&&spots.some(v=>v!==null)&&!result&&<div className="cookie-return">{counts.map((count,plate)=>count?<button type="button" key={plate} disabled={disabled} onClick={()=>put(spots.lastIndexOf(plate),null)}>↶ Take one back from {bags?'bag':'plate'} {plate+1}</button>:null)}</div>}
  {ask&&!result&&<div className="cookie-ask" role="group" aria-label={bags?'How many bags did you fill?':'How many cookies does each friend get?'}><h2>{bags?'How many bags did you fill?':'How many cookies does each friend get?'}</h2><div className="cookie-ask-choices">{ask.options.map(v=><button type="button" key={v} disabled={disabled} className="eq-answer" onClick={()=>{report('ask',String(v));answer?.(v)}}>{v}</button>)}</div></div>}
  {!ask&&<div className="cookie-check"><Button className="big-play" disabled={disabled||!!result||!full} onClick={()=>{report('check',bags?'bags':equal?'equal':'uneven');check()}}>{bags?'Check my bags →':'Check my sharing →'}</Button><span role="status" aria-live="polite">{message||(remaining?`${remaining} cookies still in the tray.`:bags?'Is every bag full?':'Look at every plate. Are they equal?')}</span></div>}
  {ask&&!result&&message&&<p className="cookie-ask-msg" role="status" aria-live="polite">{message}</p>}
  {result&&(bags?<div className="cookie-equation" aria-label={`${q.total} divided by ${q.bagSize} equals ${filled} bags`}><span className="eq-total">{q.total}</span> ÷ <span className="eq-size">{q.bagSize}</span> = <span className="eq-each">{filled}</span> <small>bags</small></div>:<div className="cookie-equation" aria-label={`${q.total} divided by ${q.plates} equals ${counts[0]}`}><span className="eq-total">{q.total}</span> ÷ <span className="eq-size">{q.plates}</span> = <span className="eq-each">{counts[0]}</span></div>)}
