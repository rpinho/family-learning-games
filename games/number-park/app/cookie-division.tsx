...
import {isDragCookie,askLine} from '@/lib/cookie-division.mjs';
...
export function CookieDivision({q,draft,disabled,result,message,helped,place,check,report,ask,answer}:Props){
 ...
 return <section className={"cookie-game"+(fixing?" cookie-game-fix":"")} aria-label="Share the cookies equally">
  <div className={'cookie-scene'+(fixing?' cookie-scene-fix':'')}><CookieBuddy shared={q.total-remaining} happy={!!result}/>{fixing&&!result&&<p className="cookie-bubble">{q.mode==='fix'?'Oops! I piled them up wrong. Can you make it fair?':'I dropped some! Count carefully and make it fair.'}</p>}{bags?<div className="cookie-count"><span className="cookie-mark" aria-hidden="true">🍪</span><strong className="eq-total">{q.total}</strong><span>cookies</span><span className="cookie-divider">·</span><strong className="eq-size">{q.bagSize}</strong><span>in each bag</span></div>:<div className="cookie-count"><span className="cookie-mark" aria-hidden="true">🍪</span><strong className="eq-total">{q.total}</strong><span>cookies</span><span className="cookie-divider">÷</span><strong className="eq-size">{q.plates}</strong><span>{rows?'rows':fixing||q.mode==='share'?'friends':'plates'}</span></div>}</div>
  ...
  {!fixing&&!ask&&spots.some(v=>v!==null)&&!result&&<div className="cookie-return">{counts.map((count,plate)=>count?<button type="button" key={plate} disabled={disabled} onClick={()=>put(spots.lastIndexOf(plate),null)}>↶ Take one back from {bags?'bag':rows?'row':'plate'} {plate+1}</button>:null)}</div>}
  {ask&&!result&&<div className="cookie-ask" role="group" aria-label={askLine(q)}><h2>{askLine(q)}</h2><div className="cookie-ask-choices">{ask.options.map(v=><button type="button" key={v} disabled={disabled} className="eq-answer" onClick={()=>{report('ask',String(v));answer?.(v)}}>{v}</button>)}</div></div>}
  ...
  {result&&!rows&&(bags?<div className="cookie-equation" aria-label={`${q.total} divided by ${q.bagSize} equals ${filled} bags`}><span className="eq-total">{q.total}</span> ÷ <span className="eq-size">{q.bagSize}</span> = <span className="eq-each">{filled}</span> <small>bags</small></div>:<div className="cookie-equation" aria-label={`${q.total} divided by ${q.plates} equals ${counts[0]}`}><span className="eq-total">{q.total}</span> ÷ <span className="eq-size">{q.plates}</span> = <span className="eq-each">{counts[0]}</span></div>)}
