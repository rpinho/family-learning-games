...
export function cookieVoiceLines(){
 ...
 for(const {plates:[minPlates,maxPlates],each:[minEach,maxEach]} of Object.values(RANGES))for(let plates=minPlates;plates<=maxPlates;plates++)for(let each=minEach;each<=maxEach;each++)lines.add(cookiePrompt(plates*each,plates));
...
export function cookieQuestion(level,random){
 ...
 return {kind:'cookies',skill:'cookies',level,mode,plan:'drag3',plates,total,baked,eaten,leftover,answer:each,max:baked,prompt:cookiePrompt(total,plates,mode,baked,eaten),fingerprint:JSON.stringify(['cookies',mode,plates,total,eaten])};
}
