...
model.evaluation={samples:results.length,top1:results.filter(x=>x.candidates[0]===x.expected).length,offered:results.filter(x=>x.label).length,correctOffered:results.filter(x=>x.label===x.expected).length,byCategory:DOODLES.map(label=>({label,total:results.filter(x=>x.expected===label).length,top1:results.filter(x=>x.expected===label&&x.candidates[0]===label).length}))};
