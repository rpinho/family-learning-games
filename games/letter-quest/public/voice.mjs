...
export class CoachVoice {
  ...
  instruction(text,{key='',essential=true}={}){
    ...
    this.stop();this.mode='instruction';const generation=this.generation;
  ...
  async play(text,generation){
    ...
    try{
      ...
      this.record('voice_play',{voice:manifest.voice,clip});
      return true;
    }catch(e){
      ...
      this.notify('Tap the speaker to hear Rook. Check the device volume if needed.');
    }
  }
}
