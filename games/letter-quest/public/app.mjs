...
function celebrate(){view='celebrate';shownBerries=-1;const recap=lessonRecap(profile);coachVoice.stop();for(const line of recap.lines)speak(line,true);app.innerHTML=`<div class="celebration">${modeBanner()}<div class="confetti">✦　✧　✦　✧</div><div class="medal">★</div><span class="eyebrow">LOOK WHAT YOU DID</span><h1>Lesson complete!</h1>${recapView(recap)}<div class="win-badges"><span>⚡ ${profile.xp} total XP</span><span>★ ${profile.completed} lessons</span></div>${coach(profile.chests?'And there’s treasure waiting for you.':'That’s a good place to pause. Or make one more move.',true)}${profile.chests?btn('Open my treasure ✧','treasure-next','button primary'):btn('Back to Learn →','exit','button primary')}${btn('Another little quest','start','text-button')}<p class="paper-prompt">✎ Bonus adventure: write one letter on paper with Mom or Dad.</p></div>`;bind();}
