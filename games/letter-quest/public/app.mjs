...
function ratingBadge(){const r=questRating(profile);return `<span class="quest-rating" title="Quest skill rating, not chess Elo. Based on practiced skill levels, not XP." aria-label="Quest rating ${r.score}, level ${r.level}"><b>♟ ${r.score}</b><small>Level ${r.level}</small></span>`;}
...
function mapView(){
 return mazeHome(profile)+learningPathView();
}
