...
test('Controls speak once per player session; manual replay and literacy targets still speak',async()=>{
 ...
 const make=()=>{const s={player:'admin',sound:true,WORDS:{start:'Controls'},sessionStorage:{getItem:k=>stored.get(k),setItem:(k,v)=>stored.set(k,v)},$:()=>node,currentChallenge:()=>cue,speak:line=>said.push(line),idleArm:()=>{},log:()=>{}};vm.createContext(s);vm.runInContext(funcs+'\nglobalThis.run=announce;',s);return s;};
