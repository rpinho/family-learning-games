...
test('Server saves separate profiles, rejects repeated arrows and never serves private logs',async t=>{
 ...
 let p=await(await post({type:'start',revision:0})).json();const target=targetsFor(p.round,0,500+FLIGHT_MS)[challengeFor(p.round,0).answerIndex],drift=aimAt(p.round,400,250,500),input={type:'shot',rules:2,motion:p.round.motion,roundId:p.round.id,shot:0,revision:p.revision,x:target.x-(drift.x-400),y:target.y-(drift.y-250),elapsed:500};
