...
test('Client touch cancellation, duplicate releases, keyboard bounds and resume use the same saved state',async()=>{
 ...
 assert.equal(stored.round.level,1);assert.equal(stored.round.readingLevel,4);assert.ok(breaks.length<=1&&breaks.every(r=>r==='mid-round'),'at most one mid-round word break in a first round');assert.equal(stored.round.mode,'learn');assert.equal(nodes.get('mode').value,'learn');
});
