...
export function question(game,level,n){
 ...
 if(game==='train'){const bank=TRAIN_SENTENCES[level-1];q.word=bank[n%bank.length];const words=tilesOf(q.word),extra=level>=2?TRAIN_DISTRACT[words.map(w=>w.toLowerCase()).find(w=>TRAIN_DISTRACT[w]&&!words.includes(TRAIN_DISTRACT[w]))]:null;q.tiles=scramble(extra?[...words,extra]:words,seeded(n*131+level));q.slots=words.length;q.answer=words.join(' ');q.prompt=`Build this sentence. ${q.word}`;q.help=q.word;}
