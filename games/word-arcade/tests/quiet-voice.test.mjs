...
import {CoachVoice,briefLine} from '../lib/voice.mjs';
const tick=()=>new Promise(resolve=>setImmediate(resolve));
test('Fast letter taps cannot erase a scheduled or playing target; success and next target remain immediate',async t=>{
 t.mock.timers.enable({apis:['Date','setTimeout'],now:1000});
 const {voice,player,events}=harness(t);
 voice.instruction('prompt',{essential:true});await voice.letter('A');
 t.mock.timers.tick(250);await tick();await voice.letter('B');assert.deepEqual(player.played,['prompt']);
 player.dispatchEvent(new Event('ended'));await voice.speak('Nice!');assert.deepEqual(player.played,['prompt','nice']);
 voice.instruction('newPrompt',{essential:true});t.mock.timers.tick(250);await tick();assert.equal(player.played.at(-1),'new');
 assert.equal(events.filter(e=>e.reason==='target_word_priority').length,2);
});
function harness(t){
